...
% automatically resolve centerline problems
    ...
    frames_around_problem_to_cut = parameters.TimeAroundProblemToCut * parameters.SampleRate;
    min_track_length = parameters.MinTrackLength * parameters.SampleRate;
    ...
    for track_index = 1:length(Tracks)
        ...
        elseif sum(potential_problems) > 0
            ...
            if sum(frames_with_potential_problems) / length(frames_with_potential_problems) > 0.10 
                %more than 10% of the frames have issues, lets throw the
                %track out
                Modifications(modifications_index).TrackIndex = track_index;
                Modifications(modifications_index).Action = 1; %delete
                modifications_index = modifications_index + 1;
    ...
    if ~isempty(Modifications)
        ...
        if parameters.TrackingDebugMode && ~isempty(deleted_tracks)
            fields_to_keep = {'Path','Size','Frames','WormIndex'};
