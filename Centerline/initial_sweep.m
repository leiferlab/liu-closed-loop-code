...
%     ConCrit = .1; %Convergence criteria
    ConCrit = .01; %Convergence criteria
    ...
    sample_size = 20; %how many straight worm images we are measuring for initialization
%     sample_size = 10; %how many straight worm images we are measuring for initialization
    min_distance_between_samples = parameters.TrackingSmoothingWindow * parameters.SampleRate;
...
%     sampled_indecies = sortIndex(1:sample_size);
    %take the samples at least some distance apart
    sampled_indecies = [];
    for sample_number = 1:sample_size
        if isempty(sortIndex)
            break
        else
            %take a sample of a frame with the highest eccentrincity
            current_sample = sortIndex(1);
            sampled_indecies = [sampled_indecies, current_sample];
            %remove frames that are within min distance from the pool
            sortIndex(and(sortIndex > current_sample - min_distance_between_samples, sortIndex < current_sample + min_distance_between_samples)) = [];
        end
    end
    if Track.Eccentricity(sampled_indecies(1)) > 0.97
        best_thresholds = 0;
    else
        best_thresholds = [];
    end
    good_frame_index = sampled_indecies(1);
    looking_for_good_frame = true; %make sure the best frame has only 2 tips
    for sample_number = 1:length(sampled_indecies)
        %this is a pretty straight worm, sample it
        index = sampled_indecies(sample_number);
...
%     l0 = median(lengths)*0.95; %the length is generally smaller than when the worm is fully extended
    l0 = prctile(lengths,25); %the length is generally smaller than when the worm is fully extended
    dilation_size = round(median(dilation_sizes));
...
%     direction_vector = [-cos([Track.Direction])'; sin([Track.Direction])'];
    ...
    potential_problems(ImageScore < 0.3) = 2;
    potential_problems(PixelsOutOfBody > 50) = 3;
    potential_problems(DisplacementScore < 0.75) = 4;
    potential_problems(Length < 15) = 5; 
