function [thinned_image_outline, thin_iteration] = find_possible_centerline_image(BW, best_thinning_iteration)
    ...
    %remove spurs
    spurless_BW = bwmorph(bwmorph(BW, 'close'), 'spur');
    if best_thinning_iteration > 2
        [first_thinned_image, ~, thin_iteration] = algbwmorph_iter_output(spurless_BW, 'thin',max(0,best_thinning_iteration-2));
    else
        [first_thinned_image, ~, thin_iteration] = algbwmorph_iter_output(spurless_BW, 'thin',2);
    end
