function [LNPStats, meanLEDPower, stdLEDPower] = directional_FitLNP_vibrations(Tracks,folder_indecies,folders)
...
%     no_weight_edges = edges(:,1:2);
    ...
    % fit the LNP models
    [LNPStats, meanLEDPower, stdLEDPower] = FitLNP_vibrations(Tracks,folder_indecies,folders);

    % update which edges are involved
    LNPStats(1).Edges = [];
    for edge_index = 1:number_of_edges
        LNPStats(edge_index).Edges = no_weight_edges(edge_index,:);
    end
