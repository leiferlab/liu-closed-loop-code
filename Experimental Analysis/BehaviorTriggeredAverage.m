function [BTA, behaviorCounts, BTA_std, BTA_stats] = BehaviorTriggeredAverage(Behaviors, LEDPowers, parameters, bootstrap)
    ...
    no_edge_Behaviors = circshift_triggers(Behaviors,BTA_seconds_before_and_after,parameters.SampleRate,false);
    Concatenated_Behaviors = horzcat(no_edge_Behaviors{:});
    
    [BTA, behaviorCounts, BTA_std] = fastparallel_BehaviorTriggeredAverage(Concatenated_Behaviors,Concatenated_LEDPowers,BTA_seconds_before_and_after,parameters.SampleRate);
    ...
    if bootstrap
        ...
        for shuffle_index = 1:number_of_random_shuffles
            %get triggers accounting for edges
            no_edge_Behaviors = circshift_triggers(Behaviors,BTA_seconds_before_and_after,parameters.SampleRate,true);
            Concatenated_Behaviors = horzcat(no_edge_Behaviors{:});
            [shuffle_BTA, ~, ~] = fastparallel_BehaviorTriggeredAverage(Concatenated_Behaviors,Concatenated_LEDPowers,BTA_seconds_before_and_after,parameters.SampleRate);
