function [LNPStats, meandutycycle, stddutycycle] = FitLNP_vibrations(Tracks,folder_indecies,folders,cross_valdiation)
...
%   Detailed explanation goes here
    ...
    all_PWMdutycycles = cell(1,length(folders));
    meanPWMdutycycle = cell(1,length(folders));
    for folder_index = 1:length(folders)
        folder_name = folders{folder_index};
        % Load Voltages
        fid = fopen([folder_name, filesep, 'PWM.txt']);
        all_PWMdutycycles{folder_index} = transpose(cell2mat(textscan(fid,'%f','HeaderLines',0,'Delimiter','\t'))); % Read data skipping header
        meanPWMdutycycle{folder_index} = mean(all_PWMdutycycles{folder_index});
        fclose(fid);
    end
    
    alldutycycle = [Tracks.PWM];
%     allLEDPower = [Tracks.LEDVoltages];
    meandutycycle = mean(alldutycycle);
    stddutycycle = std(alldutycycle);
...
%     meanLEDVoltages = mean(allLEDVoltages);
    ...
    PWM = {Tracks(:).PWM};
    [BTA, trigger_count, BTA_std, BTA_stats] = BehaviorTriggeredAverage(Behaviors, PWM, ~cross_valdiation);
    clear Behaviors PWM

    linear_kernel = BTA_to_kernel(BTA, BTA_stats, meandutycycle, ~cross_valdiation);
...
%                 all_filtered_LEDVoltages{folder_index} = conv(all_LEDVoltages{folder_index}-meanLEDVoltages{folder_index}, linear_kernel(behavior_index,:), 'same');
                all_filtered_LEDVoltages{folder_index} = padded_conv(all_PWMdutycycles{folder_index}-meanPWMdutycycle{folder_index}, linear_kernel(behavior_index,:));
            end
            
            %get all the filtered signals concatenated together
            all_filtered_signal = zeros(1, length(alldutycycle));
