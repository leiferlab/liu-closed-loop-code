function [vibration_transition_rate_of_interest,control_transition_rate_of_interest,vibration_transition_rate_of_interest_std,control_transition_rate_of_interest_std,h,p,tap_transition_total_count,control_transition_total_count,vibration_observation_total_count,control_observation_total_count] = average_transition_rate_after_vibration(folders, behavior_from, behavior_to)
...
%     time_window_after = 14; %transition rate average for 1 seconds after tap
    ...
    vibration_observation_total_count = 0;
    ...
    for folder_index = 1:length(folders)
        ...
        %normalizing to 1
        PWM_duty_cycles = load([folders{folder_index}, filesep, 'PWM.txt']);
        ...
        xcorr_ledvoltages_stimulus = padded_conv(PWM_duty_cycles, normalized_stimuli);
        peak_thresh = 0.99.*max(xcorr_ledvoltages_stimulus); %the peak threshold is 99% of the max (because edge effects)
        [~, vibration_frames] = findpeaks(xcorr_ledvoltages_stimulus, 'MinPeakHeight', peak_thresh,'MinPeakDistance',14);
        
        %generate a series of control taps
        control_frame_shift = round((vibration_frames(2)-vibration_frames(1))/2); %the control taps are exactly in between taps
        control_LEDVoltages = circshift(PWM_duty_cycles,[0,control_frame_shift]);
        ...
        for critical_frame_index = 1:length(vibration_frames)
            %for every time a stimulus is delivered, look at a certain range of
            %frames only if the track fits certain criteria
            current_critical_frame = vibration_frames(critical_frame_index);
            if current_critical_frame + time_window_after <= length(PWM_duty_cycles) && current_critical_frame - time_window_before >= 1
                ...
                if ~isempty(tracks_within_critical_window)
                    ...
                    vibration_observation_total_count = vibration_observation_total_count + size([tracks_within_critical_window.Frames],2); % keep track of how many observations we take
    ...
    for frame_index = 1:length(tap_behaviors_for_frame)
        ...
        vibration_transition_counts = [vibration_transition_counts, sum(transitions_for_frame,2)];
    end
    if isempty(vibration_transition_counts)
        tap_transition_total_count = 0;
    else
        tap_transition_total_count = sum(vibration_transition_counts(behavior_to,:),2);
    end
    vibration_transition_rate_of_interest = tap_transition_total_count./vibration_observation_total_count.*fps.*60; %convert to /min
    vibration_transition_rate_of_interest_std = sqrt(tap_transition_total_count)./vibration_observation_total_count.*fps.*60;
    ...
    control_transition_rate_of_interest_std = sqrt(tap_transition_total_count)./vibration_observation_total_count.*fps.*60;
    
    p = testPoissonSignificance(tap_transition_total_count,control_transition_total_count,vibration_observation_total_count,control_observation_total_count,0,2);
    if p > 0.05
        h = false;
    else
        h = true;
