function [] = PlotBehavioralMappingExperimentGroup (LNPStats, meanLEDPower, stdLEDPower, L, density, xx, parameters)
...
%             xlabel(['Time (s) (n=', num2str(LNPStats(behavior_index).trigger_count),')']) % x-axis label
            ...
            axis([-10,10,round(meanLEDPower)-2,round(meanLEDPower)+2])
            %axis([-10 2 0 5])
            ax = gca;
            %ax.XTick = ;
%             ax.YTick = linspace(0.64,0.84,5);
            ...
            ax.Clipping = 'off';
            xlabh = get(gca,'XLabel');
            set(xlabh,'Position',get(xlabh,'Position') + [0 6.5 0])
...
%        non_linearity_fit = fit(bin_centers',non_linearity','exp1');   %refit of necessary
        ...
        %plot non linearity
        if plot_non_linearity && ~isempty(non_linearity_fit)
